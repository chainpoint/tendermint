...
- CLI/RPC/Config
  ...
  - [rpc] [\#3471](https://github.com/chainpoint/tendermint/issues/3471) Paginate `/validators` response (default: 30 vals per page)
  - [rpc] [\#3188](https://github.com/chainpoint/tendermint/issues/3188) Remove `BlockMeta` in `ResultBlock` in favor of `BlockId` for `/block`
  - [rpc] `/block_results` response format updated (see RPC docs for details)
    ...
    ```
  - [rpc] [\#4141](https://github.com/chainpoint/tendermint/pull/4141) Remove `#event` suffix from the ID in event responses.
    `{"jsonrpc": "2.0", "id": 0, "result": ...}`
  - [rpc] [\#4141](https://github.com/chainpoint/tendermint/pull/4141) Switch to integer IDs instead of `json-client-XYZ`
    ```
  ...
  - [config] [\#4046](https://github.com/chainpoint/tendermint/issues/4046) Rename tag(s) to CompositeKey & places where tag is still present it was renamed to event or events. Find how a compositeKey is constructed [here](https://github.com/chainpoint/tendermint/blob/6d05c531f7efef6f0619155cf10ae8557dd7832f/docs/app-dev/indexing-transactions.md)
    - You will have to generate a new config for your Tendermint node(s)
  - [genesis] [\#2565](https://github.com/chainpoint/tendermint/issues/2565) Add `consensus_params.evidence.max_age_duration`. Rename
    `consensus_params.evidence.max_age` to `max_age_num_blocks`.
  - [cli] [\#1771](https://github.com/chainpoint/tendermint/issues/1771) `tendermint lite` now uses new light client package (`lite2`)
...
- Go API
  ...
  - [rpc] [\#3953](https://github.com/chainpoint/tendermint/issues/3953) Modify NewHTTP, NewXXXClient functions to return an error on invalid remote instead of panicking (@mrekucci)
  - [rpc/client] [\#3471](https://github.com/chainpoint/tendermint/issues/3471) `Validators` now requires two more args: `page` and `perPage`
  - [libs/common] [\#3262](https://github.com/chainpoint/tendermint/issues/3262) Make error the last parameter of `Task` (@PSalant726)
  - [cs/types] [\#3262](https://github.com/chainpoint/tendermint/issues/3262) Rename `GotVoteFromUnwantedRoundError` to `ErrGotVoteFromUnwantedRound` (@PSalant726)
  - [libs/common] [\#3862](https://github.com/chainpoint/tendermint/issues/3862) Remove `errors.go` from `libs/common`
  - [libs/common] [\#4230](https://github.com/chainpoint/tendermint/issues/4230) Move `KV` out of common to its own pkg
  - [libs/common] [\#4230](https://github.com/chainpoint/tendermint/issues/4230) Rename `cmn.KVPair(s)` to `kv.Pair(s)`s
  - [libs/common] [\#4232](https://github.com/chainpoint/tendermint/issues/4232) Move `Service` & `BaseService` from `libs/common` to `libs/service`
  - [libs/common] [\#4232](https://github.com/chainpoint/tendermint/issues/4232) Move `common/nil.go` to `types/utils.go` & make the functions private
  - [libs/common] [\#4231](https://github.com/chainpoint/tendermint/issues/4231) Move random functions from `libs/common` into pkg `rand`
  - [libs/common] [\#4237](https://github.com/chainpoint/tendermint/issues/4237) Move byte functions from `libs/common` into pkg `bytes`
  - [libs/common] [\#4237](https://github.com/chainpoint/tendermint/issues/4237) Move throttletimer functions from `libs/common` into pkg `timer`
  - [libs/common] [\#4237](https://github.com/chainpoint/tendermint/issues/4237) Move tempfile functions from `libs/common` into pkg `tempfile`
  - [libs/common] [\#4240](https://github.com/chainpoint/tendermint/issues/4240) Move os functions from `libs/common` into pkg `os`
  - [libs/common] [\#4240](https://github.com/chainpoint/tendermint/issues/4240) Move net functions from `libs/common` into pkg `net`
  - [libs/common] [\#4240](https://github.com/chainpoint/tendermint/issues/4240) Move mathematical functions and types out of `libs/common` to `math` pkg
  - [libs/common] [\#4240](https://github.com/chainpoint/tendermint/issues/4240) Move string functions out of `libs/common` to `strings` pkg
  - [libs/common] [\#4240](https://github.com/chainpoint/tendermint/issues/4240) Move async functions out of `libs/common` to `async` pkg
  - [libs/common] [\#4240](https://github.com/chainpoint/tendermint/issues/4240) Move bit functions out of `libs/common` to `bits` pkg
  - [libs/common] [\#4240](https://github.com/chainpoint/tendermint/issues/4240) Move cmap functions out of `libs/common` to `cmap` pkg
  - [libs/common] [\#4258](https://github.com/chainpoint/tendermint/issues/4258) Remove `Rand` from all `rand` pkg functions
  - [types] [\#2565](https://github.com/chainpoint/tendermint/issues/2565) Remove `MockBadEvidence` & `MockGoodEvidence` in favor of `MockEvidence`

- Blockchain Protocol

  - [abci] [\#2521](https://github.com/chainpoint/tendermint/issues/2521) Remove `TotalTxs` and `NumTxs` from `Header`
  - [types] [\#4151](https://github.com/chainpoint/tendermint/pull/4151) Enforce ordering of votes in DuplicateVoteEvidence to be lexicographically sorted on BlockID
  - [types] [\#1648](https://github.com/chainpoint/tendermint/issues/1648) Change `Commit` to consist of just signatures

- P2P Protocol

  - [p2p] [\#3668](https://github.com/chainpoint/tendermint/pull/3668) Make `SecretConnection` non-malleable

- [proto] [\#3986](https://github.com/chainpoint/tendermint/pull/3986) Prefix protobuf types to avoid name conflicts.
...
- [p2p] [\#4053](https://github.com/chainpoint/tendermint/issues/4053) Add `unconditional_peer_ids` and `persistent_peers_max_dial_period` config variables (see ADR-050) (@dongsam)
- [tools] [\#4227](https://github.com/chainpoint/tendermint/pull/4227) Implement `tendermint debug kill` and
...
- [cli] [\#4234](https://github.com/chainpoint/tendermint/issues/4234) Add `--db_backend and --db_dir` flags (@princesinha19)
- [cli] [\#4113](https://github.com/chainpoint/tendermint/issues/4113) Add optional `--genesis_hash` flag to check genesis hash upon startup
- [config] [\#3831](https://github.com/chainpoint/tendermint/issues/3831) Add support for [RocksDB](https://rocksdb.org/) (@Stumble)
- [rpc] [\#3985](https://github.com/chainpoint/tendermint/issues/3985) Add new `/block_by_hash` endpoint, which allows to fetch a block by its hash (@princesinha19)
- [metrics] [\#4263](https://github.com/chainpoint/tendermint/issues/4263) Add
...
- [rpc/lib] [\#4248](https://github.com/chainpoint/tendermint/issues/4248) RPC client basic authentication support (@greg-szabo)
- [lite2] [\#1771](https://github.com/chainpoint/tendermint/issues/1771) Light client with weak subjectivity

### IMPROVEMENTS:

- [rpc] [\#3188](https://github.com/chainpoint/tendermint/issues/3188) Added `block_size` to `BlockMeta` this is reflected in `/blockchain`
- [types] [\#2521](https://github.com/chainpoint/tendermint/issues/2521) Add `NumTxs` to `BlockMeta` and `EventDataNewBlockHeader`
- [p2p] [\#4185](https://github.com/chainpoint/tendermint/pull/4185) Simplify `SecretConnection` handshake with merlin
- [cli] [\#4065](https://github.com/chainpoint/tendermint/issues/4065) Add `--consensus.create_empty_blocks_interval` flag (@jgimeno)
- [docs] [\#4065](https://github.com/chainpoint/tendermint/issues/4065) Document `--consensus.create_empty_blocks_interval` flag (@jgimeno)
- [crypto] [\#4190](https://github.com/chainpoint/tendermint/pull/4190) Added SR25519 signature scheme
- [abci] [\#4177] kvstore: Return `LastBlockHeight` and `LastBlockAppHash` in `Info` (@princesinha19)
- [rpc] [\#2741](https://github.com/chainpoint/tendermint/issues/2741) Add `proposer` to `/consensus_state` response (@princesinha19)
- [deps] [\#4289](https://github.com/chainpoint/tendermint/pull/4289) Update tm-db to 0.4.0, this includes major breaking changes in the dep that change how errors are handled.

### BUG FIXES:

- [rpc/lib][\#4051](https://github.com/chainpoint/tendermint/pull/4131) Fix RPC client, which was previously resolving https protocol to http (@yenkhoon)
- [rpc] [\#4141](https://github.com/chainpoint/tendermint/pull/4141) JSONRPCClient: validate that Response.ID matches Request.ID
- [rpc] [\#4141](https://github.com/chainpoint/tendermint/pull/4141) WSClient: check for unsolicited responses
- [types] [\4164](https://github.com/chainpoint/tendermint/pull/4164) Prevent temporary power overflows on validator updates
- [cs] [\#4069](https://github.com/chainpoint/tendermint/issues/4069) Don't panic when block meta is not found in store (@gregzaitsev)
- [types] [\#4164](https://github.com/chainpoint/tendermint/issues/4164) Prevent temporary power overflows on validator updates (joint
  efforts of @gchaincl and @ancazamfir)
- [p2p] [\#4140](https://github.com/chainpoint/tendermint/issues/4140) `SecretConnection`: use the transcript solely for authentication (i.e. MAC)
- [consensus/types] [\#4243](https://github.com/chainpoint/tendermint/issues/4243) fix BenchmarkRoundStateDeepCopy panics (@cuonglm)
- [rpc] [\#4256](https://github.com/chainpoint/tendermint/issues/4256) Pass `outCapacity` to `eventBus#Subscribe` when subscribing using a local client
...
- [metrics] [\#4294](https://github.com/chainpoint/tendermint/pull/4294) Add
  - `consensus_validator_power`: track your validators power
  - `consensus_validator_last_signed_height`: track at which height the validator last signed
  - `consensus_validator_missed_blocks`: total amount of missed blocks for a validator
    as gauges in prometheus for validator specific metrics
...
- Go API
  ...
  - [libs/pubsub] [\#4070](https://github.com/chainpoint/tendermint/pull/4070) `Query#(Matches|Conditions)` returns an error.

### IMPROVEMENTS:

- [mempool] [\#4083](https://github.com/chainpoint/tendermint/pull/4083) Added TxInfo parameter to CheckTx(), and removed CheckTxWithInfo() (@erikgrinaker)
- [mempool] [\#4057](https://github.com/chainpoint/tendermint/issues/4057) Include peer ID when logging rejected txns (@erikgrinaker)
- [tools] [\#4023](https://github.com/chainpoint/tendermint/issues/4023) Improved `tm-monitor` formatting of start time and avg tx throughput (@erikgrinaker)
- [p2p] [\#3991](https://github.com/chainpoint/tendermint/issues/3991) Log "has been established or dialed" as debug log instead of Error for connected peers (@whunmr)
- [rpc] [\#4077](https://github.com/chainpoint/tendermint/pull/4077) Added support for `EXISTS` clause to the Websocket query interface.
...
- [libs/pubsub] [\#4070](https://github.com/chainpoint/tendermint/pull/4070) Strip out non-numeric characters when attempting to match numeric values.
- [libs/pubsub] [\#4070](https://github.com/chainpoint/tendermint/pull/4070) No longer panic in Query#(Matches|Conditions) preferring to return an error instead.
- [tools] [\#4023](https://github.com/chainpoint/tendermint/issues/4023) Refresh `tm-monitor` health when validator count is updated (@erikgrinaker)
- [state] [\#4104](https://github.com/chainpoint/tendermint/pull/4104) txindex/kv: Fsync data to disk immediately after receiving it (@guagualvcha)
- [state] [\#4095](https://github.com/chainpoint/tendermint/pull/4095) txindex/kv: Return an error if there's one when the user searches for a tx (hash=X) (@hsyis)
...
- [p2p] [\#4030](https://github.com/chainpoint/tendermint/issues/4030) Only allow ed25519 pubkeys when connecting
...
[\#4030](https://github.com/chainpoint/tendermint/issues/4030) for details.
...
- [p2p] [\#4030](https://github.com/chainpoint/tendermint/issues/4030) Fix for panic on nil public key send to a peer
...
- CLI/RPC/Config
  - [rpc] [\#3984](https://github.com/chainpoint/tendermint/issues/3984) Add `MempoolClient` interface to `Client` interface

### IMPROVEMENTS:

- [rpc] [\#2010](https://github.com/chainpoint/tendermint/issues/2010) Add NewHTTPWithClient and NewJSONRPCClientWithHTTPClient (note these and NewHTTP, NewJSONRPCClient functions panic if remote is invalid) (@gracenoah)
- [rpc] [\#3882](https://github.com/chainpoint/tendermint/issues/3882) Add custom marshalers to proto messages to disable `omitempty`
- [deps] [\#3952](https://github.com/chainpoint/tendermint/pull/3952) bump github.com/go-kit/kit from 0.6.0 to 0.9.0
- [deps] [\#3951](https://github.com/chainpoint/tendermint/pull/3951) bump github.com/stretchr/testify from 1.3.0 to 1.4.0
- [deps] [\#3945](https://github.com/chainpoint/tendermint/pull/3945) bump github.com/gorilla/websocket from 1.2.0 to 1.4.1
- [deps] [\#3948](https://github.com/chainpoint/tendermint/pull/3948) bump github.com/libp2p/go-buffer-pool from 0.0.1 to 0.0.2
- [deps] [\#3943](https://github.com/chainpoint/tendermint/pull/3943) bump github.com/fortytw2/leaktest from 1.2.0 to 1.3.0
- [deps] [\#3939](https://github.com/chainpoint/tendermint/pull/3939) bump github.com/rs/cors from 1.6.0 to 1.7.0
- [deps] [\#3937](https://github.com/chainpoint/tendermint/pull/3937) bump github.com/magiconair/properties from 1.8.0 to 1.8.1
- [deps] [\#3947](https://github.com/chainpoint/tendermint/pull/3947) update gogo/protobuf version from v1.2.1 to v1.3.0
- [deps] [\#4001](https://github.com/chainpoint/tendermint/pull/4001) bump github.com/tendermint/tm-db from 0.1.1 to 0.2.0

### BUG FIXES:

- [consensus] [\#3908](https://github.com/chainpoint/tendermint/issues/3908) Wait `timeout_commit` to pass even if `create_empty_blocks` is `false`
- [mempool] [\#3968](https://github.com/chainpoint/tendermint/issues/3968) Fix memory loading error on 32-bit machines (@jon-certik)
...
(gRPC)](https://github.com/chainpoint/tendermint/blob/master/docs/guides/java.md)
guide.
...
- [consensus] [\#3839](https://github.com/chainpoint/tendermint/issues/3839) Reduce "Error attempting to add vote" message severity (Error -> Info)
- [mempool] [\#3877](https://github.com/chainpoint/tendermint/pull/3877) Make `max_tx_bytes` configurable instead of `max_msg_bytes` (@bluele)
- [privval] [\#3370](https://github.com/chainpoint/tendermint/issues/3370) Refactor and simplify validator/kms connection handling. Please refer to [this comment](https://github.com/chainpoint/tendermint/pull/3370#issue-257360971) for details
- [rpc] [\#3880](https://github.com/chainpoint/tendermint/issues/3880) Document endpoints with `swagger`, introduce contract tests of implementation against documentation

### BUG FIXES:

- [config] [\#3868](https://github.com/chainpoint/tendermint/issues/3868) Move misplaced `max_msg_bytes` into mempool section (@bluele)
- [rpc] [\#3910](https://github.com/chainpoint/tendermint/pull/3910) Fix DATA RACE in HTTP client (@gchaincl)
- [store] [\#3893](https://github.com/chainpoint/tendermint/issues/3893) Fix "Unregistered interface types.Evidence" panic
...
- Go API
  - [libs] [\#3811](https://github.com/chainpoint/tendermint/issues/3811) Remove `db` from libs in favor of `https://github.com/tendermint/tm-db`

### FEATURES:

- [blockchain] [\#3561](https://github.com/chainpoint/tendermint/issues/3561) Add early version of the new blockchain reactor, which is supposed to be more modular and testable compared to the old version. To try it, you'll have to change `version` in the config file, [here](https://github.com/chainpoint/tendermint/blob/master/config/toml.go#L303) NOTE: It's not ready for a production yet. For further information, see [ADR-40](https://github.com/chainpoint/tendermint/blob/master/docs/architecture/adr-040-blockchain-reactor-refactor.md) & [ADR-43](https://github.com/chainpoint/tendermint/blob/master/docs/architecture/adr-043-blockchain-riri-org.md)
- [mempool] [\#3826](https://github.com/chainpoint/tendermint/issues/3826) Make `max_msg_bytes` configurable(@bluele)
- [node] [\#3846](https://github.com/chainpoint/tendermint/pull/3846) Allow replacing existing p2p.Reactor(s) using [`CustomReactors`
  option](https://godoc.org/github.com/chainpoint/tendermint/node#CustomReactors).
  Warning: beware of accidental name clashes. Here is the list of existing
  reactors: MEMPOOL, BLOCKCHAIN, CONSENSUS, EVIDENCE, PEX.
- [rpc] [\#3818](https://github.com/chainpoint/tendermint/issues/3818) Make `max_body_bytes` and `max_header_bytes` configurable(@bluele)
- [rpc] [\#2252](https://github.com/chainpoint/tendermint/issues/2252) Add `/broadcast_evidence` endpoint to submit double signing and other types of evidence

### IMPROVEMENTS:

- [abci] [\#3809](https://github.com/chainpoint/tendermint/issues/3809) Recover from application panics in `server/socket_server.go` to allow socket cleanup (@ruseinov)
- [p2p] [\#3664](https://github.com/chainpoint/tendermint/issues/3664) p2p/conn: reuse buffer when write/read from secret connection(@guagualvcha)
- [p2p] [\#3834](https://github.com/chainpoint/tendermint/issues/3834) Do not write 'Couldn't connect to any seeds' error log if there are no seeds in config file
- [rpc] [\#3076](https://github.com/chainpoint/tendermint/issues/3076) Improve transaction search performance

### BUG FIXES:

- [p2p] [\#3644](https://github.com/chainpoint/tendermint/issues/3644) Fix error logging for connection stop (@defunctzombie)
- [rpc] [\#3813](https://github.com/chainpoint/tendermint/issues/3813) Return err if page is incorrect (less than 0 or greater than total pages)
...
- [node][\#3730](https://github.com/chainpoint/tendermint/pull/3730) Add `CustomReactors` option to `NewNode` allowing caller to pass
  custom reactors to run inside Tendermint node (@ParthDesai)
- [abci] [\#2127](https://github.com/chainpoint/tendermint/issues/2127)RequestCheckTx has a new field, `CheckTxType`, which can take values of `CheckTxType_New` and `CheckTxType_Recheck`, indicating whether this is a new tx being checked for the first time or whether this tx is being rechecked after a block commit. This allows applications to skip certain expensive operations, like signature checking, if they've already been done once. see [docs](https://github.com/chainpoint/tendermint/blob/eddb433d7c082efbeaf8974413a36641519ee895/docs/spec/abci/apps.md#mempool-connection)

### IMPROVEMENTS:

- [rpc] [\#3700](https://github.com/chainpoint/tendermint/issues/3700) Make possible to set absolute paths for TLS cert and key (@climber73)
- [abci] [\#3513](https://github.com/chainpoint/tendermint/issues/3513) Call the reqRes callback after the resCb so they always happen in the same order

### BUG FIXES:

- [p2p] [\#3338](https://github.com/chainpoint/tendermint/issues/3338) Prevent "sent next PEX request too soon" errors by not calling
  ensurePeers outside of ensurePeersRoutine
- [behaviour] [\3772](https://github.com/chainpoint/tendermint/pull/3772) Return correct reason in MessageOutOfOrder (@jim380)
- [config] [\#3723](https://github.com/chainpoint/tendermint/issues/3723) Add consensus_params to testnet config generation; document time_iota_ms (@ashleyvega)
...
- ABCI Tags are now Events - see
  [docs](https://github.com/chainpoint/tendermint/blob/60827f75623b92eff132dc0eff5b49d2025c591e/docs/spec/abci/abci.md#events)
...
* CLI/RPC/Config
  - [cli] [\#3613](https://github.com/chainpoint/tendermint/issues/3613) Switch from golang/dep to Go Modules to resolve dependencies:
    It is recommended to switch to Go Modules if your project has tendermint as
    a dependency. Read more on Modules here:
    https://github.com/golang/go/wiki/Modules
  - [config] [\#3632](https://github.com/chainpoint/tendermint/pull/3632) Removed `leveldb` as generic
    option for `db_backend`. Must be `goleveldb` or `cleveldb`.
  - [rpc] [\#3616](https://github.com/chainpoint/tendermint/issues/3616) Fix field names for `/block_results` response (eg. `results.DeliverTx`
    -> `results.deliver_tx`). See docs for details.
  - [rpc] [\#3724](https://github.com/chainpoint/tendermint/issues/3724) RPC now binds to `127.0.0.1` by default instead of `0.0.0.0`

* Apps
  - [abci] [\#1859](https://github.com/chainpoint/tendermint/issues/1859) `ResponseCheckTx`, `ResponseDeliverTx`, `ResponseBeginBlock`,
...
* Go API
  - [abci] [\#3193](https://github.com/chainpoint/tendermint/issues/3193) Use RequestDeliverTx and RequestCheckTx in the ABCI
    Application interface
  - [libs/db] [\#3632](https://github.com/chainpoint/tendermint/pull/3632) Removed deprecated `LevelDBBackend` const
    If you have `db_backend` set to `leveldb` in your config file, please
    change it to `goleveldb` or `cleveldb`.
  - [p2p] [\#3521](https://github.com/chainpoint/tendermint/issues/3521) Remove NewNetAddressStringWithOptionalID
...
### IMPROVEMENTS:
- [abci/examples] [\#3659](https://github.com/chainpoint/tendermint/issues/3659) Change validator update tx format in the `persistent_kvstore` to use base64 for pubkeys instead of hex (@needkane)
- [consensus] [\#3656](https://github.com/chainpoint/tendermint/issues/3656) Exit if SwitchToConsensus fails
- [p2p] [\#3666](https://github.com/chainpoint/tendermint/issues/3666) Add per channel telemetry to improve reactor observability
- [rpc] [\#3686](https://github.com/chainpoint/tendermint/pull/3686) `HTTPClient#Call` returns wrapped errors, so a caller could use `errors.Cause` to retrieve an error code. (@wooparadog)

### BUG FIXES:
- [libs/db] [\#3717](https://github.com/chainpoint/tendermint/issues/3717) Fixed the BoltDB backend's Batch.Delete implementation (@Yawning)
- [libs/db] [\#3718](https://github.com/chainpoint/tendermint/issues/3718) Fixed the BoltDB backend's Get and Iterator implementation (@Yawning)
- [node] [\#3716](https://github.com/chainpoint/tendermint/issues/3716) Fix a bug where `nil` is recorded as node's address
- [node] [\#3741](https://github.com/chainpoint/tendermint/issues/3741) Fix profiler blocking the entire node
...
### BUG FIXES:
- [mempool] [\#3699](https://github.com/chainpoint/tendermint/issues/3699) Remove all committed txs from the mempool.
    ...
    See [\#3322](https://github.com/chainpoint/tendermint/issues/3322).
...
* Go API
  ...
  - [mempool, state] [\#2659](https://github.com/chainpoint/tendermint/issues/2659) `Mempool` now an interface that lives in the mempool package.
    See issue and PR for more details.
  - [p2p] [\#3346](https://github.com/chainpoint/tendermint/issues/3346) `Reactor#InitPeer` method is added to `Reactor` interface
  - [types] [\#1648](https://github.com/chainpoint/tendermint/issues/1648) `Commit#VoteSignBytes` signature was changed

### FEATURES:
- [node] [\#2659](https://github.com/chainpoint/tendermint/issues/2659) Add `node.Mempool()` method, which allows you to access mempool
- [libs/db] [\#3604](https://github.com/chainpoint/tendermint/pull/3604) Add experimental support for bolt db (etcd's fork of bolt) (@CrocdileChan)

### IMPROVEMENTS:
- [cli] [\#3585](https://github.com/chainpoint/tendermint/issues/3585) Add `--keep-addr-book` option to `unsafe_reset_all` cmd to not
  clear the address book (@climber73)
- [cli] [\#3160](https://github.com/chainpoint/tendermint/issues/3160) Add
  `--config=<path-to-config>` option to `testnet` cmd (@gregdhill)
- [cli] [\#3661](https://github.com/chainpoint/tendermint/pull/3661) Add
...
- [crypto] [\#3672](https://github.com/chainpoint/tendermint/issues/3672) Return more info in the `AddSignatureFromPubKey` error
- [cs/replay] [\#3460](https://github.com/chainpoint/tendermint/issues/3460) Check appHash for each block
- [libs/db] [\#3611](https://github.com/chainpoint/tendermint/issues/3611) Conditional compilation
...
- [node] [\#3362](https://github.com/chainpoint/tendermint/issues/3362) Return an error if `persistent_peers` list is invalid (except
  when IP lookup fails)
- [p2p] [\#3463](https://github.com/chainpoint/tendermint/pull/3463) Do not log "Can't add peer's address to addrbook" error for a private peer (@guagualvcha)
- [p2p] [\#3531](https://github.com/chainpoint/tendermint/issues/3531) Terminate session on nonce wrapping (@climber73)
- [pex] [\#3647](https://github.com/chainpoint/tendermint/pull/3647) Dial seeds, if any, instead of crawling peers first (@defunctzombie)
- [rpc] [\#3534](https://github.com/chainpoint/tendermint/pull/3534) Add support for batched requests/responses in JSON RPC
- [rpc] [\#3362](https://github.com/chainpoint/tendermint/issues/3362) `/dial_seeds` & `/dial_peers` return errors if addresses are
  incorrect (except when IP lookup fails)

### BUG FIXES:
- [consensus] [\#3067](https://github.com/chainpoint/tendermint/issues/3067) Fix replay from appHeight==0 with validator set changes (@james-ray)
- [consensus] [\#3304](https://github.com/chainpoint/tendermint/issues/3304) Create a peer state in consensus reactor before the peer
  is started (@guagualvcha)
- [lite] [\#3669](https://github.com/chainpoint/tendermint/issues/3669) Add context parameter to RPC Handlers in proxy routes (@yutianwu)
- [mempool] [\#3322](https://github.com/chainpoint/tendermint/issues/3322) When a block is committed, only remove committed txs from the mempool
that were valid (ie. `ResponseDeliverTx.Code == 0`)
- [p2p] [\#3338](https://github.com/chainpoint/tendermint/issues/3338) Ensure `RemovePeer` is always called before `InitPeer` (upon a peer
  reconnecting to our node)
- [p2p] [\#3532](https://github.com/chainpoint/tendermint/issues/3532) Limit the number of attempts to connect to a peer in seed mode
  to 16 (as a result, the node will stop retrying after a 35 hours time window)
- [p2p] [\#3362](https://github.com/chainpoint/tendermint/issues/3362) Allow inbound peers to be persistent, including for seed nodes.
- [pex] [\#3603](https://github.com/chainpoint/tendermint/pull/3603) Dial seeds when addrbook needs more addresses (@defunctzombie)
...
- [state] [\#3537](https://github.com/chainpoint/tendermint/pull/3537#issuecomment-482711833)
  `LoadValidators`: do not return an empty validator set
- [blockchain] [\#3457](https://github.com/chainpoint/tendermint/issues/3457)
...
Also, it contains an [ADR](https://github.com/chainpoint/tendermint/pull/3539) that proposes decoupling the
...
- [p2p] [\#3463](https://github.com/chainpoint/tendermint/pull/3463) Do not log "Can't add peer's address to addrbook" error for a private peer
- [p2p] [\#3547](https://github.com/chainpoint/tendermint/pull/3547) Fix a couple of annoying typos (@mdyring)

### BUG FIXES:

- [docs] [\#3514](https://github.com/chainpoint/tendermint/issues/3514) Fix block.Header.Time description (@melekes)
- [p2p] [\#2716](https://github.com/chainpoint/tendermint/issues/2716) Check if we're already connected to peer right before dialing it (@melekes)
- [p2p] [\#3545](https://github.com/chainpoint/tendermint/issues/3545) Add back `NetAddress()` to `NodeInfo` and use it instead of peer's `SocketAddr()` when adding a peer to the `PEXReactor` (potential fix for [\#3532](https://github.com/chainpoint/tendermint/issues/3532))
- [state] [\#3438](https://github.com/chainpoint/tendermint/pull/3438)
...
### BREAKING CHANGES:
* Go API
  - [crypto/secp256k1] [\#3439](https://github.com/chainpoint/tendermint/issues/3439)
...
- [p2p] [\#3522](https://github.com/chainpoint/tendermint/issues/3522) Memoize
...
* Go API
  - [libs/autofile] [\#3504](https://github.com/chainpoint/tendermint/issues/3504) Remove unused code in autofile package. Deleted functions: `Group.Search`, `Group.FindLast`, `GroupReader.ReadLine`, `GroupReader.PushLine`, `MakeSimpleSearchFunc` (@guagualvcha)
...
- [circle] [\#3497](https://github.com/chainpoint/tendermint/issues/3497) Move release management to CircleCI

### BUG FIXES:

- [mempool] [\#3512](https://github.com/chainpoint/tendermint/issues/3512) Fix panic from concurrent access to txsMap, a regression for external ABCI apps introduced in v0.31.1
...
* Go API
  - [crypto] [\#3426](https://github.com/chainpoint/tendermint/pull/3426) Remove `Ripemd160` helper method (@needkane)
  - [libs/common] [\#3429](https://github.com/chainpoint/tendermint/pull/3429) Remove `RepeatTimer` (also `TimerMaker` and `Ticker` interface)
  - [rpc/client] [\#3458](https://github.com/chainpoint/tendermint/issues/3458) Include `NetworkClient` interface into `Client` interface
  - [types] [\#3448](https://github.com/chainpoint/tendermint/issues/3448) Remove method `PB2TM.ConsensusParams`
...
### FEATURES:
 ...
 - [rpc] [\#3419](https://github.com/chainpoint/tendermint/issues/3419) Start HTTPS server if `rpc.tls_cert_file` and `rpc.tls_key_file` are provided in the config (@guagualvcha)

### IMPROVEMENTS:

- [docs] [\#3140](https://github.com/chainpoint/tendermint/issues/3140) Formalize proposer election algorithm properties
- [docs] [\#3482](https://github.com/chainpoint/tendermint/issues/3482) Fix broken links (@brapse)
- [mempool] [\#2778](https://github.com/chainpoint/tendermint/issues/2778) No longer send txs back to peers who sent it to you.
...
For instance, for a 4 node localnet, in a test sending 250byte txs for 120 sec. at 500 txs/sec (total of 15MB):
  ...
  - total bytes sent to 1st node:
     ...
     - after: 19304964 (19MB)
- [p2p] [\#3475](https://github.com/chainpoint/tendermint/issues/3475) Simplify `GetSelectionWithBias` for addressbook (@guagualvcha)
- [rpc/lib/client] [\#3430](https://github.com/chainpoint/tendermint/issues/3430) Disable compression for HTTP client to prevent GZIP-bomb DoS attacks (@guagualvcha)

### BUG FIXES:

- [blockchain] [\#2699](https://github.com/chainpoint/tendermint/issues/2699) Update the maxHeight when a peer is removed
- [mempool] [\#3478](https://github.com/chainpoint/tendermint/issues/3478) Fix memory-leak related to `broadcastTxRoutine` (@HaoyangLiu)
...
See the [v0.31.0
Milestone](https://github.com/chainpoint/tendermint/milestone/19?closed=1) for
more details.
...
* CLI/RPC/Config
  - [config] [\#2920](https://github.com/chainpoint/tendermint/issues/2920) Remove `consensus.blocktime_iota` parameter
  - [rpc] [\#3227](https://github.com/chainpoint/tendermint/issues/3227) New PubSub design does not block on clients when publishing
    messages. Slow clients may miss messages and receive an error, terminating
    the subscription.
  - [rpc] [\#3269](https://github.com/chainpoint/tendermint/issues/2826) Limit number of unique clientIDs with open subscriptions. Configurable via `rpc.max_subscription_clients`
  - [rpc] [\#3269](https://github.com/chainpoint/tendermint/issues/2826) Limit number of unique queries a given client can subscribe to at once. Configurable via `rpc.max_subscriptions_per_client`.
  - [rpc] [\#3435](https://github.com/chainpoint/tendermint/issues/3435) Default ReadTimeout and WriteTimeout changed to 10s. WriteTimeout can increased by setting `rpc.timeout_broadcast_tx_commit` in the config.
  - [rpc/client] [\#3269](https://github.com/chainpoint/tendermint/issues/3269) Update `EventsClient` interface to reflect new pubsub/eventBus API [ADR-33](https://github.com/chainpoint/tendermint/blob/develop/docs/architecture/adr-033-pubsub.md). This includes `Subscribe`, `Unsubscribe`, and `UnsubscribeAll` methods.

* Apps
  - [abci] [\#3403](https://github.com/chainpoint/tendermint/issues/3403) Remove `time_iota_ms` from BlockParams. This is a
    ConsensusParam but need not be exposed to the app for now.
  - [abci] [\#2920](https://github.com/chainpoint/tendermint/issues/2920) Rename `consensus_params.block_size` to `consensus_params.block` in ABCI ConsensusParams

* Go API
  - [libs/common] TrapSignal accepts logger as a first parameter and does not block anymore
    * previously it was dumping "captured ..." msg to os.Stdout
    * TrapSignal should not be responsible for blocking thread of execution
  - [libs/db] [\#3397](https://github.com/chainpoint/tendermint/pull/3397) Add possibility to `Close()` `Batch` to prevent memory leak when using ClevelDB. (@Stumble)
  - [types] [\#3354](https://github.com/chainpoint/tendermint/issues/3354) Remove RoundState from EventDataRoundState
  - [rpc] [\#3435](https://github.com/chainpoint/tendermint/issues/3435) `StartHTTPServer` / `StartHTTPAndTLSServer` now require a Config (use `rpcserver.DefaultConfig`)
...
### FEATURES:
- [config] [\#3269](https://github.com/chainpoint/tendermint/issues/2826) New configuration values for controlling RPC subscriptions:
...
- [types] [\#2920](https://github.com/chainpoint/tendermint/issues/2920) Add `time_iota_ms` to block's consensus parameters (not exposed to the application)
- [lite] [\#3269](https://github.com/chainpoint/tendermint/issues/3269) Add `/unsubscribe_all` endpoint to unsubscribe from all events
- [mempool] [\#3079](https://github.com/chainpoint/tendermint/issues/3079) Bound mempool memory usage via the `mempool.max_txs_bytes` configuration value. Set to 1GB by default. The mempool's current `txs_total_bytes` is exposed via `total_bytes` field in
  `/num_unconfirmed_txs` and `/unconfirmed_txs` RPC endpoints.

### IMPROVEMENTS:
- [all] [\#3385](https://github.com/chainpoint/tendermint/issues/3385), [\#3386](https://github.com/chainpoint/tendermint/issues/3386) Various linting improvements
- [crypto] [\#3371](https://github.com/chainpoint/tendermint/issues/3371) Copy in secp256k1 package from go-ethereum instead of importing
  go-ethereum (@silasdavis)
- [deps] [\#3382](https://github.com/chainpoint/tendermint/issues/3382) Don't pin repos without releases
- [deps] [\#3357](https://github.com/chainpoint/tendermint/issues/3357), [\#3389](https://github.com/chainpoint/tendermint/issues/3389), [\#3392](https://github.com/chainpoint/tendermint/issues/3392) Update gogo/protobuf, golang/protobuf, levigo, golang.org/x/crypto
- [libs/common] [\#3238](https://github.com/chainpoint/tendermint/issues/3238) exit with zero (0) code upon receiving SIGTERM/SIGINT
- [libs/db] [\#3378](https://github.com/chainpoint/tendermint/issues/3378) CLevelDB#Stats now returns the following properties:
  ...
  - leveldb.alivesnaps
  - leveldb.aliveiters
- [privval] [\#3351](https://github.com/chainpoint/tendermint/pull/3351) First part of larger refactoring that clarifies and separates concerns in the privval package.

### BUG FIXES:
- [blockchain] [\#3358](https://github.com/chainpoint/tendermint/pull/3358) Fix timer leak in `BlockPool` (@guagualvcha)
- [cmd] [\#3408](https://github.com/chainpoint/tendermint/issues/3408) Fix `testnet` command's panic when creating non-validator configs (using `--n` flag) (@srmo)
- [libs/db/remotedb/grpcdb] [\#3402](https://github.com/chainpoint/tendermint/issues/3402) Close Iterator/ReverseIterator after use
- [libs/pubsub] [\#951](https://github.com/chainpoint/tendermint/issues/951), [\#1880](https://github.com/chainpoint/tendermint/issues/1880) Use non-blocking send when dispatching messages [ADR-33](https://github.com/chainpoint/tendermint/blob/develop/docs/architecture/adr-033-pubsub.md)
- [lite] [\#3364](https://github.com/chainpoint/tendermint/issues/3364) Fix `/validators` and `/abci_query` proxy endpoints
  (@guagualvcha)
- [p2p/conn] [\#3347](https://github.com/chainpoint/tendermint/issues/3347) Reject all-zero shared secrets in the Diffie-Hellman step of secret-connection
- [p2p] [\#3369](https://github.com/chainpoint/tendermint/issues/3369) Do not panic when filter times out
- [p2p] [\#3359](https://github.com/chainpoint/tendermint/pull/3359) Fix reconnecting report duplicate ID error due to race condition between adding peer to peerSet and starting it (@guagualvcha)
...
* [config] [\#3291](https://github.com/chainpoint/tendermint/issues/3291) Make
  config.ResetTestRootWithChainID() create concurrency-safe test directories.

### BUG FIXES:

* [consensus] [\#3295](https://github.com/chainpoint/tendermint/issues/3295)
  Flush WAL on stop to prevent data corruption during graceful shutdown.
* [consensus] [\#3302](https://github.com/chainpoint/tendermint/issues/3302)
  Fix possible halt by resetting TriggeredTimeoutPrecommit before starting next height.
* [rpc] [\#3251](https://github.com/chainpoint/tendermint/issues/3251) Fix
  `/net_info#peers#remote_ip` format. New format spec:
  * dotted decimal ("192.0.2.1"), if ip is an IPv4 or IP4-mapped IPv6 address
  * IPv6 ("2001:db8::1"), if ip is a valid IPv6 address
* [cmd] [\#3314](https://github.com/chainpoint/tendermint/issues/3314) Return
  an error on `show_validator` when the private validator file does not exist.
* [p2p] [\#3010](https://github.com/chainpoint/tendermint/issues/3010#issuecomment-464287627)
...
the application ([\#3285](https://github.com/chainpoint/tendermint/issues/3285)), and even to support randomness ([\#763](https://github.com/chainpoint/tendermint/issues/763)).
For more, see issues marked
[proposer-selection](https://github.com/chainpoint/tendermint/labels/proposer-selection).
...
* Apps
  - [state] [\#3222](https://github.com/chainpoint/tendermint/issues/3222)
...
* Go API
  - [types] [\#3222](https://github.com/chainpoint/tendermint/issues/3222)
...
* Block Protocol
  - [state] [\#3286](https://github.com/chainpoint/tendermint/issues/3286) Blocks that include already committed evidence are invalid.

* P2P Protocol
  - [consensus] [\#3222](https://github.com/chainpoint/tendermint/issues/3222)
...
### IMPROVEMENTS:
- [crypto] [\#3279](https://github.com/chainpoint/tendermint/issues/3279) Use `btcec.S256().N` directly instead of hard coding a copy.

### BUG FIXES:
- [state] [\#3222](https://github.com/chainpoint/tendermint/issues/3222) Fix validator set updates so they are applied as a set, rather
...
- [evidence] [\#3286](https://github.com/chainpoint/tendermint/issues/3286) Don't add committed evidence to evidence pool.
...
- p2p:
  - Partial fix for MITM attacks on the p2p connection. MITM conditions may
    still exist. See [\#3010](https://github.com/chainpoint/tendermint/issues/3010).
- crypto:
...
* Go API
  - [crypto] [\#3278](https://github.com/chainpoint/tendermint/issues/3278) Remove
    MixEntropy functions
  - [types] [\#3245](https://github.com/chainpoint/tendermint/issues/3245) Commit uses `type CommitSig Vote` instead of `Vote` directly.
    In preparation for removing redundant fields from the commit [\#1648](https://github.com/chainpoint/tendermint/issues/1648)

### IMPROVEMENTS:
- [consensus] [\#3246](https://github.com/chainpoint/tendermint/issues/3246) Better logging and notes on recovery for corrupted WAL file
- [crypto] [\#3163](https://github.com/chainpoint/tendermint/issues/3163) Use ethereum's libsecp256k1 go-wrapper for signatures when cgo is available
- [crypto] [\#3162](https://github.com/chainpoint/tendermint/issues/3162) Wrap btcd instead of forking it to keep up with fixes (used if cgo is not available)
- [makefile] [\#3233](https://github.com/chainpoint/tendermint/issues/3233) Use golangci-lint instead of go-metalinter
- [tools] [\#3218](https://github.com/chainpoint/tendermint/issues/3218) Add go-deadlock tool to help detect deadlocks
- [tools] [\#3106](https://github.com/chainpoint/tendermint/issues/3106) Add tm-signer-harness test harness for remote signers
- [tests] [\#3258](https://github.com/chainpoint/tendermint/issues/3258) Fixed a bunch of non-deterministic test failures

### BUG FIXES:
- [node] [\#3186](https://github.com/chainpoint/tendermint/issues/3186) EventBus and indexerService should be started before first block (for replay last block on handshake) execution (@ackratos)
- [p2p] [\#3232](https://github.com/chainpoint/tendermint/issues/3232) Fix infinite loop leading to addrbook deadlock for seed nodes
- [p2p] [\#3247](https://github.com/chainpoint/tendermint/issues/3247) Fix panic in SeedMode when calling FlushStop and OnStop
  concurrently
- [p2p] [\#3040](https://github.com/chainpoint/tendermint/issues/3040) Fix MITM on secret connection by checking low-order points
- [privval] [\#3258](https://github.com/chainpoint/tendermint/issues/3258) Fix race between sign requests and ping requests in socket that was causing messages to be corrupted
...
### IMPROVEMENTS:
- [pex] [\#3037](https://github.com/chainpoint/tendermint/issues/3037) Only log "Reached max attempts to dial" once
- [rpc] [\#3159](https://github.com/chainpoint/tendermint/issues/3159) Expose
  `triggered_timeout_commit` in the `/dump_consensus_state`

### BUG FIXES:
- [consensus] [\#3199](https://github.com/chainpoint/tendermint/issues/3199) Fix consensus halt with no empty blocks from not resetting triggeredTimeoutCommit
- [p2p] [\#2967](https://github.com/chainpoint/tendermint/issues/2967) Fix file descriptor leak
...
@ancazamfir](https://github.com/chainpoint/tendermint/pull/3140).
...
That said, we just discovered [another bug](https://github.com/chainpoint/tendermint/issues/3181),
...
* Apps
  - [state] [\#3049](https://github.com/chainpoint/tendermint/issues/3049) Total voting power of the validator set is upper bounded by
    `MaxInt64 / 8`. Apps must ensure they do not return changes to the validator
    set that cause this maximum to be exceeded.

* Go API
  - [node] [\#3082](https://github.com/chainpoint/tendermint/issues/3082) MetricsProvider now requires you to pass a chain ID
  - [types] [\#2713](https://github.com/chainpoint/tendermint/issues/2713) Rename `TxProof.LeafHash` to `TxProof.Leaf`
  - [crypto/merkle] [\#2713](https://github.com/chainpoint/tendermint/issues/2713) `SimpleProof.Verify` takes a `leaf` instead of a
    `leafHash` and performs the hashing itself

* Blockchain Protocol
  * [crypto/merkle] [\#2713](https://github.com/chainpoint/tendermint/issues/2713) Merkle trees now match the RFC 6962 specification
  * [types] [\#3078](https://github.com/chainpoint/tendermint/issues/3078) Re-order Timestamp and BlockID in CanonicalVote so it's
    consistent with CanonicalProposal (BlockID comes
    first)
  * [types] [\#3165](https://github.com/chainpoint/tendermint/issues/3165) Hash of ConsensusParams only includes BlockSize.MaxBytes and
    BlockSize.MaxGas

* P2P Protocol
  - [consensus] [\#3049](https://github.com/chainpoint/tendermint/issues/3049) Normalize priorities to not exceed `2*TotalVotingPower` to mitigate unfair proposer selection
    heavily preferring earlier joined validators in the case of an early bonded large validator unbonding

### FEATURES:

### IMPROVEMENTS:
- [rpc] [\#3065](https://github.com/chainpoint/tendermint/issues/3065) Return maxPerPage (100), not defaultPerPage (30) if `per_page` is greater than the max 100.
- [instrumentation] [\#3082](https://github.com/chainpoint/tendermint/issues/3082) Add `chain_id` label for all metrics

### BUG FIXES:
- [crypto] [\#3164](https://github.com/chainpoint/tendermint/issues/3164) Update `btcd` fork for rare signRFC6979 bug
- [lite] [\#3171](https://github.com/chainpoint/tendermint/issues/3171) Fix verifying large validator set changes
- [log] [\#3125](https://github.com/chainpoint/tendermint/issues/3125) Fix year format
- [mempool] [\#3168](https://github.com/chainpoint/tendermint/issues/3168) Limit tx size to fit in the max reactor msg size
- [scripts] [\#3147](https://github.com/chainpoint/tendermint/issues/3147) Fix json2wal for large block parts (@bradyjoestar)
...
* CLI/RPC/Config
  ...
  - [config] [\#2992](https://github.com/chainpoint/tendermint/issues/2992) `allow_duplicate_ip` is now set to false
  - [privval] [\#1181](https://github.com/chainpoint/tendermint/issues/1181) Split `priv_validator.json` into immutable (`config/priv_validator_key.json`) and mutable (`data/priv_validator_state.json`) parts (@yutianwu)
  - [privval] [\#2926](https://github.com/chainpoint/tendermint/issues/2926) Split up `PubKeyMsg` into `PubKeyRequest` and `PubKeyResponse` to be consistent with other message types
  - [privval] [\#2923](https://github.com/chainpoint/tendermint/issues/2923) Listen for unix socket connections instead of dialing them

* Apps

* Go API
  - [types] [\#2981](https://github.com/chainpoint/tendermint/issues/2981) Remove `PrivValidator.GetAddress()`
...
### FEATURES:
- [rpc] [\#3052](https://github.com/chainpoint/tendermint/issues/3052) Include peer's remote IP in `/net_info`

### IMPROVEMENTS:
- [consensus] [\#3086](https://github.com/chainpoint/tendermint/issues/3086) Log peerID on ignored votes (@srmo)
- [docs] [\#3061](https://github.com/chainpoint/tendermint/issues/3061) Added specification for signing consensus msgs at
  ./docs/spec/consensus/signing.md
- [privval] [\#2948](https://github.com/chainpoint/tendermint/issues/2948) Memoize pubkey so it's only requested once on startup
- [privval] [\#2923](https://github.com/chainpoint/tendermint/issues/2923) Retry RemoteSigner connections on error

### BUG FIXES:

- [build] [\#3085](https://github.com/chainpoint/tendermint/issues/3085) Fix `Version` field in build scripts (@husio)
- [crypto/multisig] [\#3102](https://github.com/chainpoint/tendermint/issues/3102) Fix multisig keys address length
- [crypto/encoding] [\#3101](https://github.com/chainpoint/tendermint/issues/3101) Fix `PubKeyMultisigThreshold` unmarshalling into `crypto.PubKey` interface
- [p2p/conn] [\#3111](https://github.com/chainpoint/tendermint/issues/3111) Make SecretConnection thread safe
- [rpc] [\#3053](https://github.com/chainpoint/tendermint/issues/3053) Fix internal error in `/tx_search` when results are empty
  (@gianfelipe93)
- [types] [\#2926](https://github.com/chainpoint/tendermint/issues/2926) Do not panic if retrieving the privval's public key fails
...
- [mempool] [\#3036](https://github.com/chainpoint/tendermint/issues/3036) Fix
...
* Go API
  - [dep] [\#3027](https://github.com/chainpoint/tendermint/issues/3027) Revert to mainline Go crypto library, eliminating the modified
...
- [node] [\#3025](https://github.com/chainpoint/tendermint/issues/3025) Validate NodeInfo addresses on startup.

### BUG FIXES:

- [p2p] [\#3025](https://github.com/chainpoint/tendermint/pull/3025) Revert to using defers in addrbook.  Fixes deadlocks in pex and consensus upon invalid ExternalAddr/ListenAddr configuration.
...
### FEATURES:
- [rpc] [\#2964](https://github.com/chainpoint/tendermint/issues/2964) Add `UnconfirmedTxs(limit)` and `NumUnconfirmedTxs()` methods to HTTP/Local clients (@danil-lashin)
- [docs] [\#3004](https://github.com/chainpoint/tendermint/issues/3004) Enable full-text search on docs pages

### IMPROVEMENTS:
- [consensus] [\#2971](https://github.com/chainpoint/tendermint/issues/2971) Return error if ValidatorSet is empty after InitChain
  (@leo-xinwang)
- [ci/cd] [\#3005](https://github.com/chainpoint/tendermint/issues/3005) Updated CircleCI job to trigger website build when docs are updated
- [docs] Various updates

### BUG FIXES:
- [cmd] [\#2983](https://github.com/chainpoint/tendermint/issues/2983) `testnet` command always sets `addr_book_strict = false`
- [config] [\#2980](https://github.com/chainpoint/tendermint/issues/2980) Fix CORS options formatting
- [kv indexer] [\#2912](https://github.com/chainpoint/tendermint/issues/2912) Don't ignore key when executing CONTAINS
- [mempool] [\#2961](https://github.com/chainpoint/tendermint/issues/2961) Call `notifyTxsAvailable` if there're txs left after committing a block, but recheck=false
- [mempool] [\#2994](https://github.com/chainpoint/tendermint/issues/2994) Reject txs with negative GasWanted
- [p2p] [\#2990](https://github.com/chainpoint/tendermint/issues/2990) Fix a bug where seeds don't disconnect from a peer after 3h
- [consensus] [\#3006](https://github.com/chainpoint/tendermint/issues/3006) Save state after InitChain only when stateHeight is also 0 (@james-ray)
...
issue](https://github.com/chainpoint/tendermint/issues/2718#issuecomment-440888677)
...
* CLI/RPC/Config
  - [rpc] [\#2932](https://github.com/chainpoint/tendermint/issues/2932) Rename `accum` to `proposer_priority`

* Go API
  - [db] [\#2913](https://github.com/chainpoint/tendermint/pull/2913)
    ReverseIterator API change: start < end, and end is exclusive.
  - [types] [\#2932](https://github.com/chainpoint/tendermint/issues/2932) Rename `Validator.Accum` to `Validator.ProposerPriority`

* Blockchain Protocol
  - [state] [\#2714](https://github.com/chainpoint/tendermint/issues/2714) Validators can now only use pubkeys allowed within
    ConsensusParams.Validator.PubKeyTypes

* P2P Protocol
  - [consensus] [\#2871](https://github.com/chainpoint/tendermint/issues/2871)
    Remove *ProposalHeartbeat* message as it serves no real purpose (@srmo)
  - [state] Fixes for proposer selection:
    - [\#2785](https://github.com/chainpoint/tendermint/issues/2785) Accum for new validators is `-1.125*totalVotingPower` instead of 0
    - [\#2941](https://github.com/chainpoint/tendermint/issues/2941) val.Accum is preserved during ValidatorSet.Update to avoid being
      reset to 0

### IMPROVEMENTS:

- [state] [\#2929](https://github.com/chainpoint/tendermint/issues/2929) Minor refactor of updateState logic (@danil-lashin)
- [node] [\#2959](https://github.com/chainpoint/tendermint/issues/2959) Allow node to start even if software's BlockProtocol is
  different from state's BlockProtocol
- [pex] [\#2959](https://github.com/chainpoint/tendermint/issues/2959) Pex reactor logger uses `module=pex`

### BUG FIXES:

- [p2p] [\#2968](https://github.com/chainpoint/tendermint/issues/2968) Panic on transport error rather than continuing to run but not
  accept new connections
- [p2p] [\#2969](https://github.com/chainpoint/tendermint/issues/2969) Fix mismatch in peer count between `/net_info` and the prometheus
  metrics
- [rpc] [\#2408](https://github.com/chainpoint/tendermint/issues/2408) `/broadcast_tx_commit`: Fix "interface conversion: interface {} in nil, not EventDataTx" panic (could happen if somebody sent a tx using `/broadcast_tx_commit` while Tendermint was being stopped)
- [state] [\#2785](https://github.com/chainpoint/tendermint/issues/2785) Fix accum for new validators to be `-1.125*totalVotingPower`
  instead of 0, forcing them to wait before becoming the proposer. Also:
    - do not batch clip
    - keep accums averaged near 0
- [txindex/kv] [\#2925](https://github.com/chainpoint/tendermint/issues/2925) Don't return false positives when range searching for a prefix of a tag value
- [types] [\#2938](https://github.com/chainpoint/tendermint/issues/2938) Fix regression in v0.26.4 where we panic on empty
  genDoc.Validators
- [types] [\#2941](https://github.com/chainpoint/tendermint/issues/2941) Preserve val.Accum during ValidatorSet.Update to avoid it being
...
- [rpc] [\#2747](https://github.com/chainpoint/tendermint/issues/2747) Enable subscription to tags emitted from `BeginBlock`/`EndBlock` (@kostko)
- [types] [\#2747](https://github.com/chainpoint/tendermint/issues/2747) Add `ResultBeginBlock` and `ResultEndBlock` fields to `EventDataNewBlock`
    and `EventDataNewBlockHeader` to support subscriptions (@kostko)
- [types] [\#2918](https://github.com/chainpoint/tendermint/issues/2918) Add Marshal, MarshalTo, Unmarshal methods to various structs
  to support Protobuf compatibility (@nagarajmanjunath)

### IMPROVEMENTS:

- [config] [\#2877](https://github.com/chainpoint/tendermint/issues/2877) Add `blocktime_iota` to the config.toml (@ackratos)
    - NOTE: this should be a ConsensusParam, not part of the config, and will be
      removed from the config at a later date
      ([\#2920](https://github.com/chainpoint/tendermint/issues/2920).
- [mempool] [\#2882](https://github.com/chainpoint/tendermint/issues/2882) Add txs from Update to cache
- [mempool] [\#2891](https://github.com/chainpoint/tendermint/issues/2891) Remove local int64 counter from being stored in every tx
- [node] [\#2866](https://github.com/chainpoint/tendermint/issues/2866) Add ability to instantiate IPCVal (@joe-bowman)

### BUG FIXES:

- [blockchain] [\#2731](https://github.com/chainpoint/tendermint/issues/2731) Retry both blocks if either is bad to avoid getting stuck during fast sync (@goolAdapter)
- [consensus] [\#2893](https://github.com/chainpoint/tendermint/issues/2893) Use genDoc.Validators instead of state.NextValidators on replay when appHeight==0 (@james-ray)
- [log] [\#2868](https://github.com/chainpoint/tendermint/issues/2868) Fix `module=main` setting overriding all others
...
- [rpc] [\#2808](https://github.com/chainpoint/tendermint/issues/2808) Fix `accum` field in `/validators` by calling `IncrementAccum` if necessary
- [rpc] [\#2811](https://github.com/chainpoint/tendermint/issues/2811) Allow integer IDs in JSON-RPC requests (@tomtau)
- [txindex/kv] [\#2759](https://github.com/chainpoint/tendermint/issues/2759) Fix tx.height range queries
- [txindex/kv] [\#2775](https://github.com/chainpoint/tendermint/issues/2775) Order tx results by index if height is the same
- [txindex/kv] [\#2908](https://github.com/chainpoint/tendermint/issues/2908) Don't return false positives when searching for a prefix of a tag value
...
* Go API
  - [rpc] [\#2791](https://github.com/chainpoint/tendermint/issues/2791) Functions that start HTTP servers are now blocking:
  ...
  - [rpc] [\#2767](https://github.com/chainpoint/tendermint/issues/2767) Subscribing to events
...
- [log] [\#2843](https://github.com/chainpoint/tendermint/issues/2843) New `log_format` config option, which can be set to 'plain' for colored
  text or 'json' for JSON output
- [types] [\#2767](https://github.com/chainpoint/tendermint/issues/2767) New event types EventDataNewRound (with ProposerInfo) and EventDataCompleteProposal (with BlockID). (@kevlubkcm)

### IMPROVEMENTS:

- [dep] [\#2844](https://github.com/chainpoint/tendermint/issues/2844) Dependencies are no longer pinned to an exact version in the
  Gopkg.toml:
  - Serialization libs are allowed to vary by patch release
  - Other libs are allowed to vary by minor release
- [p2p] [\#2857](https://github.com/chainpoint/tendermint/issues/2857) "Send failed" is logged at debug level instead of error.
- [rpc] [\#2780](https://github.com/chainpoint/tendermint/issues/2780) Add read and write timeouts to HTTP servers
- [state] [\#2848](https://github.com/chainpoint/tendermint/issues/2848) Make "Update to validators" msg value pretty (@danil-lashin)

### BUG FIXES:
- [consensus] [\#2819](https://github.com/chainpoint/tendermint/issues/2819) Don't send proposalHearbeat if not a validator
- [docs] [\#2859](https://github.com/chainpoint/tendermint/issues/2859) Fix ConsensusParams details in spec
- [libs/autofile] [\#2760](https://github.com/chainpoint/tendermint/issues/2760) Comment out autofile permissions check - should fix
  running Tendermint on Windows
- [p2p] [\#2869](https://github.com/chainpoint/tendermint/issues/2869) Set connection config properly instead of always using default
- [p2p/pex] [\#2802](https://github.com/chainpoint/tendermint/issues/2802) Seed mode fixes:
...
- [rpc] [\#2582](https://github.com/chainpoint/tendermint/issues/2582) Enable CORS on RPC API (@hleb-albau)

### BUG FIXES:

- [abci] [\#2748](https://github.com/chainpoint/tendermint/issues/2748) Unlock mutex in localClient so even when app panics (e.g. during CheckTx), consensus continue working
- [abci] [\#2748](https://github.com/chainpoint/tendermint/issues/2748) Fix DATA RACE in localClient
- [amino] [\#2822](https://github.com/chainpoint/tendermint/issues/2822) Update to v0.14.1 to support compiling on 32-bit platforms
- [rpc] [\#2748](https://github.com/chainpoint/tendermint/issues/2748) Drain channel before calling Unsubscribe(All) in `/broadcast_tx_commit`
...
- [consensus] [\#2704](https://github.com/chainpoint/tendermint/issues/2704) Simplify valid POL round logic
- [docs] [\#2749](https://github.com/chainpoint/tendermint/issues/2749) Deduplicate some ABCI docs
- [mempool] More detailed log messages
    - [\#2724](https://github.com/chainpoint/tendermint/issues/2724)
    - [\#2762](https://github.com/chainpoint/tendermint/issues/2762)

### BUG FIXES:

- [autofile] [\#2703](https://github.com/chainpoint/tendermint/issues/2703) Do not panic when checking Head size
- [crypto/merkle] [\#2756](https://github.com/chainpoint/tendermint/issues/2756) Fix crypto/merkle ProofOperators.Verify to check bounds on keypath parts.
- [mempool] fix a bug where we create a WAL despite `wal_dir` being empty
- [p2p] [\#2771](https://github.com/chainpoint/tendermint/issues/2771) Fix `peer-id` label name to `peer_id` in prometheus metrics
- [p2p] [\#2797](https://github.com/chainpoint/tendermint/pull/2797) Fix IDs in peer NodeInfo and require them for addresses
  in AddressBook
- [p2p] [\#2797](https://github.com/chainpoint/tendermint/pull/2797) Do not close conn immediately after sending pex addrs in seed mode. Partial fix for [\#2092](https://github.com/chainpoint/tendermint/issues/2092).
...
* CLI/RPC/Config
  * [config] [\#2232](https://github.com/chainpoint/tendermint/issues/2232) Timeouts are now strings like "3s" and "100ms", not ints
  * [config] [\#2505](https://github.com/chainpoint/tendermint/issues/2505) Remove Mempool.RecheckEmpty (it was effectively useless anyways)
  * [config] [\#2490](https://github.com/chainpoint/tendermint/issues/2490) `mempool.wal` is disabled by default
  * [privval] [\#2459](https://github.com/chainpoint/tendermint/issues/2459) Split `SocketPVMsg`s implementations into Request and Response, where the Response may contain a error message (returned by the remote signer)
  * [state] [\#2644](https://github.com/chainpoint/tendermint/issues/2644) Add Version field to State, breaking the format of State as
    encoded on disk.
  * [rpc] [\#2298](https://github.com/chainpoint/tendermint/issues/2298) `/abci_query` takes `prove` argument instead of `trusted` and switches the default
    behaviour to `prove=false`
  * [rpc] [\#2654](https://github.com/chainpoint/tendermint/issues/2654) Remove all `node_info.other.*_version` fields in `/status` and
    `/net_info`
  * [rpc] [\#2636](https://github.com/chainpoint/tendermint/issues/2636) Remove
    `_params` suffix from fields in `consensus_params`.

* Apps
  * [abci] [\#2298](https://github.com/chainpoint/tendermint/issues/2298) ResponseQuery.Proof is now a structured merkle.Proof, not just
    arbitrary bytes
  * [abci] [\#2644](https://github.com/chainpoint/tendermint/issues/2644) Add Version to Header and shift all fields by one
  * [abci] [\#2662](https://github.com/chainpoint/tendermint/issues/2662) Bump the field numbers for some `ResponseInfo` fields to make room for
      `AppVersion`
  * [abci] [\#2636](https://github.com/chainpoint/tendermint/issues/2636) Updates to ConsensusParams
...
* Go API
  * [config] [\#2232](https://github.com/chainpoint/tendermint/issues/2232) Timeouts are time.Duration, not ints
  * [crypto/merkle & lite] [\#2298](https://github.com/chainpoint/tendermint/issues/2298) Various changes to accomodate General Merkle trees
  * [crypto/merkle] [\#2595](https://github.com/chainpoint/tendermint/issues/2595) Remove all Hasher objects in favor of byte slices
  * [crypto/merkle] [\#2635](https://github.com/chainpoint/tendermint/issues/2635) merkle.SimpleHashFromTwoHashes is no longer exported
  * [node] [\#2479](https://github.com/chainpoint/tendermint/issues/2479) Remove node.RunForever
  * [rpc/client] [\#2298](https://github.com/chainpoint/tendermint/issues/2298) `ABCIQueryOptions.Trusted` -> `ABCIQueryOptions.Prove`
  * [types] [\#2298](https://github.com/chainpoint/tendermint/issues/2298) Remove `Index` and `Total` fields from `TxProof`.
  * [types] [\#2598](https://github.com/chainpoint/tendermint/issues/2598)
    `VoteTypeXxx` are now of type `SignedMsgType byte` and named `XxxType`, eg.
    `PrevoteType`, `PrecommitType`.
  * [types] [\#2636](https://github.com/chainpoint/tendermint/issues/2636) Rename fields in ConsensusParams to remove `Params` suffixes
  * [types] [\#2735](https://github.com/chainpoint/tendermint/issues/2735) Simplify Proposal message to align with spec

* Blockchain Protocol
  * [crypto/tmhash] [\#2732](https://github.com/chainpoint/tendermint/issues/2732) TMHASH is now full 32-byte SHA256
    * All hashes in the block header and Merkle trees are now 32-bytes
    * PubKey Addresses are still only 20-bytes
  * [state] [\#2587](https://github.com/chainpoint/tendermint/issues/2587) Require block.Time of the fist block to be genesis time
  * [state] [\#2644](https://github.com/chainpoint/tendermint/issues/2644) Require block.Version to match state.Version
  * [types] Update SignBytes for `Vote`/`Proposal`/`Heartbeat`:
    * [\#2459](https://github.com/chainpoint/tendermint/issues/2459) Use amino encoding instead of JSON in `SignBytes`.
    * [\#2598](https://github.com/chainpoint/tendermint/issues/2598) Reorder fields and use fixed sized encoding.
    * [\#2598](https://github.com/chainpoint/tendermint/issues/2598) Change `Type` field from `string` to `byte` and use new
      `SignedMsgType` to enumerate.
  * [types] [\#2730](https://github.com/chainpoint/tendermint/issues/2730) Use
    same order for fields in `Vote` as in the SignBytes
  * [types] [\#2732](https://github.com/chainpoint/tendermint/issues/2732) Remove the address field from the validator hash
  * [types] [\#2644](https://github.com/chainpoint/tendermint/issues/2644) Add Version struct to Header
  * [types] [\#2609](https://github.com/chainpoint/tendermint/issues/2609) ConsensusParams.Hash() is the hash of the amino encoded
    struct instead of the Merkle tree of the fields
  * [types] [\#2670](https://github.com/chainpoint/tendermint/issues/2670) Header.Hash() builds Merkle tree out of fields in the same
    order they appear in the header, instead of sorting by field name
  * [types] [\#2682](https://github.com/chainpoint/tendermint/issues/2682) Use proto3 `varint` encoding for ints that are usually unsigned (instead of zigzag encoding).
  * [types] [\#2636](https://github.com/chainpoint/tendermint/issues/2636) Add Validator field to ConsensusParams
      (Used to control which pubkey types validators can use, by abci type).

* P2P Protocol
  * [consensus] [\#2652](https://github.com/chainpoint/tendermint/issues/2652)
    Replace `CommitStepMessage` with `NewValidBlockMessage`
  * [consensus] [\#2735](https://github.com/chainpoint/tendermint/issues/2735) Simplify `Proposal` message to align with spec
  * [consensus] [\#2730](https://github.com/chainpoint/tendermint/issues/2730)
    Add `Type` field to `Proposal` and use same order of fields as in the
    SignBytes for both `Proposal` and `Vote`
  * [p2p] [\#2654](https://github.com/chainpoint/tendermint/issues/2654) Add `ProtocolVersion` struct with protocol versions to top of
    DefaultNodeInfo and require `ProtocolVersion.Block` to match during peer handshake


### FEATURES:
- [abci] [\#2557](https://github.com/chainpoint/tendermint/issues/2557) Add `Codespace` field to `Response{CheckTx, DeliverTx, Query}`
- [abci] [\#2662](https://github.com/chainpoint/tendermint/issues/2662) Add `BlockVersion` and `P2PVersion` to `RequestInfo`
- [crypto/merkle] [\#2298](https://github.com/chainpoint/tendermint/issues/2298) General Merkle Proof scheme for chaining various types of Merkle trees together
- [docs/architecture] [\#1181](https://github.com/chainpoint/tendermint/issues/1181) S
...
- [config] [\#2232](https://github.com/chainpoint/tendermint/issues/2232) Added ValidateBasic method, which performs basic checks
- [crypto/ed25519] [\#2558](https://github.com/chainpoint/tendermint/issues/2558) Switch to use latest `golang.org/x/crypto` through our fork at
  github.com/tendermint/crypto
- [libs/log] [\#2707](https://github.com/chainpoint/tendermint/issues/2707) Add year to log format (@yutianwu)
...
- Merge `tendermint/go-p2p -> chainpoint/tendermint/p2p` and `tendermint/go-rpc -> chainpoint/tendermint/rpc/lib`
...
- Docker image improved, slimmed down (using Alpine), and changed from tendermint/tmbase to chainpoint/tendermint

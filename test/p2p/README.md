...
For consistency, we assume all commands are run from the Tendermint repository root (ie. $GOPATH/src/github.com/chainpoint/tendermint).
...
for i in $(seq 1 4); do
	docker run -d \
	  ...
	  -e TMHOME=/go/src/github.com/chainpoint/tendermint/test/p2p/data/mach$((i-1)) \
